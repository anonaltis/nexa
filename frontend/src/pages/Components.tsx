...
const Components = () => {
  ...
  const handleAiRecommend = async () => {
    ...
    setIsAiLoading(true);
    try {
      toast.info("Component Agent: Searching smart recommendations...");
      const response = await api.get(`/components?q=${aiPrompt}`);
      setComponents(response.data);
      toast.success(`Smart Registry Update: ${response.data.length} potential matches`);
    } catch (error) {
      toast.error("Component Agent: Recommendation link failed");
    } finally {
      ...
      <div className="container max-w-7xl mx-auto px-4 py-12 space-y-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-9 space-y-8">
            ...
            <div className="blueprint-card p-6 border-primary/20 bg-primary/5 space-y-4">
              <div className="flex items-center gap-2">
                <Sparkles className="w-4 h-4 text-primary animate-pulse" />
                <h3 className="text-[10px] font-black uppercase tracking-[0.3em] text-primary">AI_SMART_RECOMMENDER</h3>
              </div>
              <p className="text-[9px] text-muted-foreground uppercase font-bold tracking-widest opacity-60">
                Describe your project requirements to find optimal components.
              </p>
              <div className="flex gap-4">
                <div className="relative flex-1">
                  <input
                    ...
                    placeholder="E.G. HIGH_GAIN_OPAMP_FOR_AUDIO_PREAMP..."
                    className="w-full bg-background/50 border border-primary/10 rounded h-12 px-4 text-[10px] font-bold uppercase tracking-widest focus:outline-none focus:border-primary/40 transition-colors placeholder:opacity-20"
                  />
                  <div className="absolute right-3 top-1/2 -translate-y-1/2">
                    <Wand2 className="w-4 h-4 text-primary/30" />
                  </div>
                </div>
                <Button
                  onClick={handleAiRecommend}
                  disabled={isAiLoading || !aiPrompt.trim()}
                  className="h-12 px-8 bg-primary/20 hover:bg-primary/30 border border-primary/30 text-[10px] font-black uppercase tracking-widest"
                >
                  {isAiLoading ? "Processing..." : "Search_Intelligence"}
                </Button>
              </div>
            </div>
